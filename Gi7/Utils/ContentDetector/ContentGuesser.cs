...
namespace Gi7.Utils.ContentDetector
{
    public class ContentGuesser
    {
        ...
        public HeaderSignature GuessType(String filename, byte[] data)
        {
            ...
            foreach (var header in HeaderSignature.StockSignatures)
            {
                if (MatchesFileExtension(header.FileExtensions, extension)) {
                    return header;
                }
            }
            ...
            var realData = data.Take(100).ToArray();
            foreach (var header in HeaderSignature.StockSignatures)
            {
                var score = MatchesSignature(header.Signature, realData);
        ...
        private bool MatchesFileExtension(string[] extensions, string extension)
        {
            ...
            else
            {
                foreach (string fileExtension in extensions)
                {
                    if (string.Compare(
                        extension,
                        fileExtension.Trim('.'), StringComparison.CurrentCultureIgnoreCase) == 0)
                    {
                        return true;
                    }
                }
